  ...
  if (ytMatch && ytMatch[1]) {
    return (
      <div className="mt-4 rounded-2xl overflow-hidden aspect-video border-4 border-pink-50 relative shadow-sm">
        <iframe src={`https://www.youtube.com/embed/${ytMatch[1]}`} className="absolute top-0 left-0 w-full h-full" allowFullScreen title="YouTube Video"></iframe>
      </div>
    );
  }
  ...
  if (igMatch && igMatch[1]) {
    return (
      <div className="mt-4 rounded-2xl overflow-hidden bg-white border-4 border-pink-50 flex justify-center shadow-sm">
        <iframe src={`https://www.instagram.com/p/${igMatch[1]}/embed`} className="w-full max-w-[400px] h-[480px]" frameBorder="0" scrolling="no" allowTransparency title="Instagram Post"></iframe>
      </div>
    );
  }
...
const FallingCheese = () => {
  ...
  useEffect(() => {
    const newCheeses = Array.from({ length: 12 }).map((_, i) => ({
      id: i, left: Math.random() * 100, animationDuration: Math.random() * 6 + 6, delay: Math.random() * 5, size: Math.random() * 10 + 15,
    }));
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden opacity-60">
      {cheeses.map((cheese) => (
        <motion.div key={cheese.id} initial={{ y: -50, opacity: 0, rotate: 0 }} animate={{ y: "110vh", opacity: [0, 1, 1, 0], rotate: 360 }} transition={{ duration: cheese.animationDuration, repeat: Infinity, delay: cheese.delay, ease: "linear" }} style={{ position: "absolute", left: `${cheese.left}%`, fontSize: `${cheese.size}px` }}>
          🧀
        </motion.div>
      ))}
    </div>
          ...
          <div className="relative -mt-16 p-1.5 rounded-full bg-gradient-to-tr from-yellow-300 via-pink-300 to-pink-400 mb-3 shadow-lg z-10">
            <div className="w-[110px] h-[110px] bg-white rounded-full p-1">
              <div className="w-full h-full bg-pink-50 rounded-full flex items-center justify-center text-4xl overflow-hidden">
                <img src={profile?.acf?.avatar || "https://api.dicebear.com/7.x/notionists/svg?seed=Cheese&backgroundColor=ffd5dc"} alt="Avatar của Cheese" className="w-full h-full object-cover" />
              </div>
            </div>
          ...
          <div className="flex gap-3 w-full px-5">
            ...
            {profile?.acf?.link_tiktok && (
              <a href={profile.acf.link_tiktok} target="_blank" rel="noreferrer" className="flex-1 flex items-center justify-center gap-2 bg-slate-800 text-white py-2.5 rounded-xl font-bold text-sm hover:bg-slate-700 transition shadow-sm">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18V5l12-2v13"/><path d="m9 9 12-2"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg> TikTok
              </a>
            )}
